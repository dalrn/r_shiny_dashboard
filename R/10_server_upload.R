...
# TUJUAN: Server logic untuk upload dan explorasi data (modul)
# ============================================================================
...
server_upload <- function(input, output, session, rv) {
  ...
  output$data_loaded_msg <- renderUI({
    ...
    create_success_message(
      glue::glue("Upload BERHASIL: {length(rv$value_col)} observasi | Frequency: {rv$data_frequency} | Seasonal: {if(rv$is_seasonal) 'YA' else 'TIDAK'}")
    )
  })
  ...
  output$data_summary <- renderPrint({
    ...
    cat("Seasonal:", if (rv$is_seasonal) paste("YA, Period =", rv$seasonal_period) else "TIDAK", "\n")
  })
  ...
  output$auto_detection_info <- renderPrint({
    ...
    cat("Seasonality:", if (rv$is_seasonal) paste("YA, Periode =", rv$seasonal_period) else "TIDAK", "\n")
  })
  ...
  output$plot_initial <- renderPlotly({
    ...
    plotly::plot_ly(df, x = ~Time, y = ~Value, type = "scatter", mode = "lines",
      ...
      plotly::add_trace(y = ~Trend, name = "Trend", mode = "lines",
