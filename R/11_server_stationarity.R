...
# TUJUAN: Server logic untuk stationarity testing (modul)
# ============================================================================
...
server_stationarity <- function(input, output, session, rv) {
  ...
  output$adf_test_original <- renderPrint({
    ...
    tryCatch({
      ...
      cat("Statistik Uji:", format_number(rv$adf_result_original$statistic), "\n")
      cat("P-value:", format(rv$adf_result_original$p_value, scientific = TRUE, digits = 4), "\n")
      cat("Kesimpulan:", rv$adf_result_original$interpretation, "\n\n")
  ...
  output$adf_interpretation_original <- renderUI({
    ...
    if (rv$adf_result_original$is_stationary) {
      create_success_message(
        paste("Data STASIONER (p-value =", format(rv$adf_result_original$p_value, digits = 4), ")")
      )
    } else {
      create_error_message(
        paste("❌ Data TIDAK STASIONER (p-value =", format(rv$adf_result_original$p_value, digits = 4), ")")
      )
    }
  })
  ...
  output$adf_test_differenced <- renderPrint({
    ...
    tryCatch({
      ...
      cat("Statistik Uji:", format_number(rv$adf_result_differenced$statistic), "\n")
      cat("P-value:", format(rv$adf_result_differenced$p_value, scientific = TRUE, digits = 4), "\n")
      cat("Kesimpulan:", rv$adf_result_differenced$interpretation, "\n\n")
  ...
  output$adf_interpretation_differenced <- renderUI({
    ...
    if (rv$adf_result_differenced$is_stationary) {
      create_success_message(
        paste("STASIONER (p-value =", format(rv$adf_result_differenced$p_value, digits = 4), ")")
      )
    } else {
      create_error_message(
        paste("❌ TIDAK STASIONER (p-value =", format(rv$adf_result_differenced$p_value, digits = 4), ")")
      )
    }
  })
  ...
  output$plot_acf <- renderPlot({
    if (is.null(rv$ts_differenced)) {
      plot.new()
      text(0.5, 0.5, "Pilih tingkat differencing untuk menampilkan ACF plot",
    ...
    tryCatch({
      if (length(rv$ts_differenced) < 2) {
        plot.new()
        text(0.5, 0.5, "Tidak cukup data untuk ACF plot",
  ...
  output$plot_pacf <- renderPlot({
    if (is.null(rv$ts_differenced)) {
      plot.new()
      text(0.5, 0.5, "Pilih tingkat differencing untuk menampilkan PACF plot",
           cex = 1.2, col = "gray50", adj = c(0.5, 0.5))
      return()
    }
    ...
    tryCatch({
      if (length(rv$ts_differenced) < 2) {
        plot.new()
        text(0.5, 0.5, "Tidak cukup data untuk PACF plot",
             cex = 1.2, col = "red", adj = c(0.5, 0.5))
        return()
      }
